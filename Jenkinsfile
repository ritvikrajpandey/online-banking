pipeline {
    agent any
    parameters {
        string(name: 'MYSQL_ROOT_PASSWORD', defaultValue: 'K9039680300p', description: 'MySQL password')
    }
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                ...
                sh 'docker build -t ritvikrajpandey/online-banking:1 .'
            }
        }
       stage('Create Database') {
        ...
        stage('Deploy and Run') {
            steps {
                ...
                sh 'docker run --detach --name=cloudbank -p 8888:8888 --link bankmysql:localhost -t ritvikrajpandey/online-banking:1'
            }
        }
    }
}